...
//
float data_hlt_e[9] = {-180, -120, -60, -30, 0, 30, 60, 120, 180};
float data_hlt_e_dot[3] = {-10, 0, 10};

//
float data_kp[7] = {4, 5, 6, 7, 8, 9, 10};
float data_ki[7] = {10, 20, 30, 40, 50, 60, 70};
float data_kd[7] = {0.09, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03};
...
void value_hlt_e(float* data_hlt_e, float* muy_e, float e)
{
    if (e < data_hlt_e[0])
		{
        muy_e[0] = 1;
...
void value_hlt_e_dot(float* data_hlt_e_dot, float* muy_e_dot, float e_dot)
{
...
void find_beta(float* muy_e, float* muy_e_dot, float* beta)
{
   int a = 0;
   for(int i = 0;i < 3;i++)
	 {
...
void rule_fuzzy(float* rule_kp, float* rule_ki, float* rule_kd, float* data_kp, float* data_ki, float* data_kd)
{																		
    rule_kp[0] = data_kp[0];
    rule_kp[1] = data_kp[1];
    rule_kp[2] = data_kp[2];
...
float defuzzy_kp(float* beta, float* rule_kp)
{
...
float defuzzy_ki(float* beta, float* rule_ki)
{
    float temp1=0, temp2=0;
    for(int i=0;i<27;i++)
		{
...
float defuzzy_kd(float* beta, float* rule_kd)
{
    float temp1=0, temp2=0;
    for(int i=0;i<27;i++)
		{
